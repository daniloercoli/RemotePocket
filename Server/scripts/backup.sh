...
recent_weekly=$(find "$backup_dir" -type f -name 'mydesk_backup_*_weekly.sql.gz' -mtime -7 -print)
[ -n "$recent_weekly" ] || kind=weekly
backup_file="$backup_dir/mydesk_backup_$(date -u +%Y%m%d_%H%M%S)_$$_${kind}.sql.gz"
partial_file="$backup_file.partial"
docker compose -f docker-compose.prod.yml exec -T postgres \
    pg_dump --no-owner --no-privileges -U mydesk_user -d mydesk_prod | gzip > "$partial_file"
gzip -t "$partial_file"
...
find "$backup_dir" -type f -name 'mydesk_backup_*_daily.sql.gz' -mtime +"$retention_daily_days" -delete
find "$backup_dir" -type f -name 'mydesk_backup_*_weekly.sql.gz' -mtime +"$retention_weekly_days" -delete
